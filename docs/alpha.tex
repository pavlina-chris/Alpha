...
As a separate coercion rule, the binary logical operators should coerce both
operands to $\R{B}$.
...
The subtraction operator is ``\texttt{-}''. When both operands are numeric, it
applies \textref{ssb:types:casting:coercion}{standard coercion rules}, then
performs subtraction. When both operands are pointers of the same type, it
subtracts the pointers and divides by the width of the subtype, yielding a
value of type $\R{SI}_{\textrm{pointer width}}$ (\texttt{ssize}). This means
that if \texttt{p} and \texttt{q} point to two elements in a sequence,
...
time. For primitive values, \texttt{x~op=~y} should behave identically to
\texttt{x~=~x~op~y}. For non-primitives, the operation is allowed to actually
modify the object. For example, when performing array concatenation, the
intermediate value may be optimised away, and the right-hand values added
directly to a resized left-hand array.
