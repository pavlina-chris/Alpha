...
def build_jar_file (root, inputs, mainclass, filename, verbose):
    cwd = os.getcwd ()
    os.chdir (root)
    cmdline = ['jar', 'cf', filename] + inputs
    if verbose:
        print (" ".join (cmdline))
    else:
        print (filename)
...
def main ():
    if "--help" in sys.argv:
        print ("Compiles JCommander. Options:")
        print ("  --help")
        print ("  --verbose")
    ...
    java_files = recurse (
            ...
            compile_if_needed,
            [do_java_compile],
            [".java", ".class", ["src/main/java"], verbose]
            )
...
#            )
    ...
    if java_files:
        ...
        try:
            ...
            pass
        prep_jar_directory ("build/jar", ["src/main/java"], [], verbose)

        # Build the jar file
        build_jar_file ("build/jar", ["com"], None,
                "jcommander.jar", verbose)

    # Move/copy files out
    if java_files:
        os.rename ("build/jar/jcommander.jar", "jcommander.jar")
#    if c_files:
